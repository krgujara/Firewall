...
public class Firewall {
	 ...
	 * 
	 * @param rulesFile
	 *            (string): path to the csv files, from where the rules are to be
	 *            loaded.
	 */
	...
	/**
	 * Function that takes exactly four arguments and returns a boolean: true, if
	 * there exists a rule in the file that this object was initialized with that
	 * allows traffic with these particular properties, and false otherwise.
	 * 
	 * @param direction
	 *            (string): “inbound” or “outbound”
	 * @param protocol
	 *            (string): exactly one of “tcp” or “udp”, all lowercase
	 * @param port
	 *            (integer) – an integer in the range [1, 65535]
	 * @param ip_address
	 *            (string): a single well-formed IPv4 address.
	...
	public synchronized boolean accept_packet(String direction, String protocol, Integer port, String ipAddress) {
		...
		try 
		{
			...
			// divide the allowedRules list into chunks, and send it to the seperate threads
			// to execute
	...
	// runnable class which actually does all the required processing. 
