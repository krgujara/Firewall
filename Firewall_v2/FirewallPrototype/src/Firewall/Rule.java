...
public class Rule {
	 ...
	 * 
	 * @param direction
	 *            (directions enum): values can be inbound/ outbound
	 * @param protocol
	 *            (protocols enum): values can be tcp/ udp
	 * @param portBegin
	 *            (int): port value/ first value of the range if port is given as a
	 *            range
	 * @param portEnd
	 *            (int): second value of the range if port is given as a range, 0
	 *            otherwise
	 * @param ipAddressBegin
	 *            (String): ip value/ first value of the range if ipAddress is given
	 *            as a range
	 * @param ipAddressEnd
	 *            (String): ip value/ second value of the range if ipAddress is
	 *            given as a range
	 */
	...
	public boolean equals(final Object obj) {
		...
		if (portEnd == 0) {
			if (portBegin != other.portBegin)
				return false;
		} else if ((other.portBegin < portBegin || portEnd < other.portEnd)) // check if port number falls in the range
																				// [portBegin, portEnd]
			return false;
